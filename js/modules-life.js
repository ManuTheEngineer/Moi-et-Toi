...
function loadCycleData() {
  ...
  db.ref('herWellness/cycle')
    ...
    .once('value', snap => {
      ...
      if (dayOfCycle <= 5) {
        phase = 'Menstrual';
        info = "Rest is productive right now. Honor your body's need to slow down. Iron-rich foods help.";
      } else if (dayOfCycle <= 13) {
        phase = 'Follicular';
        info = 'Energy is rising. Great time for new projects, socializing, and trying new things.';
      } else if (dayOfCycle <= 16) {
        phase = 'Ovulatory';
        info = 'Peak energy and confidence. Communication skills are sharpest now.';
      } else if (dayOfCycle <= 28) {
        phase = 'Luteal';
        info = 'Winding down. Focus on completion, not starting new things. Self-care is essential.';
      } else {
        phase = 'Cycle day ' + dayOfCycle;
        info = "If your cycle is longer than 28 days, that's normal. Everyone's different.";
      }
...
function listenPersonalGoals(who) {
  ...
  db.ref('personalGoals/' + who)
    ...
    .on('value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">What are you working towards?</div>';
        return;
      }
      el.innerHTML = items
        .map(i => {
...
function renderHabits(habits) {
  ...
  if (!el) return;
  if (!habits.length) {
    el.innerHTML = '<div class="empty">Build consistency together</div>';
    return;
  }
...
function nextDeepTalk() {
  dtIndex++;
  loadDeepTalk();
}
function prevDeepTalk() {
  dtIndex = Math.max(0, dtIndex - 1);
  loadDeepTalk();
}
...
async function saveConvoNote() {
  ...
  if (!text) {
    toast('Write a reflection first');
    return;
  }
  const btn = event?.target;
  if (btn) {
...
function listenConvoNotes() {
  ...
  fbOn(ref, 'value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">After a deep conversation, write what you learned.</div>';
        return;
      }
      el.innerHTML = items
        .map(i => {
...
function listenBabyNames() {
  db.ref('family/names')
    ...
    .on('value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">Save names you both love</div>';
        return;
      }
...
function listenFamilyGoals() {
  db.ref('family/goals')
    ...
    .on('value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">Set goals for your future family</div>';
        return;
      }
      el.innerHTML = items
        .map(
...
function listenValues() {
  db.ref('foundation/values')
    ...
    .on('value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">What do you both stand for?</div>';
        return;
      }
      const icons = ['◆', '✦', '♡', '⬡', '◇', '✧', '●', '○'];
      el.innerHTML = items
...
function listenPrayers() {
  db.ref('spiritual/prayers')
    ...
    .on('value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">Share what you need prayer for</div>';
        return;
      }
      el.innerHTML = items
        .map(i => {
...
function listenBlessings() {
  db.ref('spiritual/blessings')
    ...
    .on('value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">Count your blessings together</div>';
        return;
      }
      el.innerHTML = items
        .map(i => {
...
function listenIntentions() {
  db.ref('spiritual/intentions')
    ...
    .on('value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">Set spiritual intentions together</div>';
        return;
      }
      el.innerHTML = items
        .map(i => {
...
function renderFinRecent(expenses) {
  ...
  const catIcons = {
    ...
    transport: _s(
      '<path d="M5 17h14v-5H5zm0 0a2 2 0 01-2-2V8l2-4h14l2 4v7a2 2 0 01-2 2m-12 0v2m10-2v2"/><circle cx="7.5" cy="13" r="1"/><circle cx="16.5" cy="13" r="1"/>'
    ),
    housing: _s('<path d="M3 9.5L12 3l9 6.5V20a1 1 0 01-1 1H4a1 1 0 01-1-1V9.5z"/><path d="M9 21V14h6v7"/>'),
    entertainment: _s(
    ...
    subscriptions: _s('<rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12.01" y2="18"/>'),
    other: _s(
...
function renderFinCatChart(expenses) {
  ...
  const colors = {
    ...
    transport: '#4A90D9',
    housing: '#2D8A56',
    entertainment: '#7B5B8C',
    shopping: '#D4943A',
    health: '#C44B4B',
    subscriptions: '#3A7BA6',
    other: '#6B6465'
  };
...
async function addSavingsGoal() {
  ...
  if (!name || !target) {
    toast('Name and target required');
    return;
  }
  const btn = event?.target;
  if (btn) {
...
function listenSavings() {
  db.ref('homelife/savings')
    ...
    .on('value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">Set savings goals together</div>';
        return;
      }
      el.innerHTML = items
        .map(i => {
...
function updateSavings(key) {
  ...
  `);
  setTimeout(function () {
    var el = document.getElementById('savings-amount-input');
    if (el) el.focus();
  }, 300);
}
...
function listenChores() {
  db.ref('homelife/chores')
    ...
    .on('value', snap => {
      ...
      if (!el) return;
      if (!items.length) {
        el.innerHTML = '<div class="empty">Divide and conquer household tasks</div>';
        return;
      }
      const whoLabels = {
        shared: 'Shared',
